...
def simulate_worker_upload(num_tasks=10, num_workers=25):
    ...
    # （可选强烈建议）快速自检，确保尺寸正确：
    # assert len(raw_weights) == num_workers == 25
    # assert len(raw_worker_locs) == len(raw_max_dists) == num_workers
    # assert len(raw_task_locs) == num_tasks == 10
    # assert len(raw_costs) == num_tasks and all(len(r) == num_workers for r in raw_costs)  # 这行在“转置之后”检查
    # assert len(raw_quals) == num_tasks and all(len(r) == num_workers for r in raw_quals)
...
def main():
    ...
    for _row_i, row in df_cfg.iterrows():
        ...
        # 执行每组参数 5 次
        for run in range(20):  # 内部循环，执行 5 次
            print(f"执行第 {run + 1} 次...")
            # 清空旧文件
