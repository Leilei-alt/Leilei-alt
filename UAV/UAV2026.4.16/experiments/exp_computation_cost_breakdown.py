...
def main() -> None:
    ...
    for i in range(trials):
        row = run_one_trial(task_type="VIDEO_ANALYSIS", task_size=64)
        row["trial"] = i
        rows.append(row)
        print(f"[Trial {i}] vehicle={row['vehicle_ms']:.3f}ms server={row['server_ms']:.3f}ms ta={row['ta_ms']:.3f}ms total={row['total_ms']:.3f}ms")

    summary_rows = summarize(rows)
    write_csv(RESULTS_DIR / "comp_breakdown_raw.csv", rows)
    write_csv(RESULTS_DIR / "comp_breakdown_summary.csv", summary_rows)
