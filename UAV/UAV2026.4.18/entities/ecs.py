...
class OffloadPacket:
    ...
    def summary(self) -> str:
        return (
            f"OffloadPacket(task_type={self.task_type}, task_size={self.task_size}, "
            f"payload_digest={self.payload_digest[:16]}..., auth_tag={self.auth_tag[:16]}...)"
        )
...
class ECS:
    ...
    def establish_internal_channel(self, aas_id: str, nonce_a: int, nonce_e: int) -> str:
        self.current_sk_ae = kdf("SK_AE", aas_id, self.ecs_id, self.domain_id, str(nonce_a), str(nonce_e))
    ...
    def issue_service_challenge(self, timestamp: int) -> ServiceChallenge:
        ...
        return ServiceChallenge(ecs_id=self.ecs_id, nonce_e=nonce_e, timestamp=timestamp)
