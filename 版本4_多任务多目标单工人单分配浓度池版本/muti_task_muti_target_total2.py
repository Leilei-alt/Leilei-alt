...
def simulate_worker_upload1(num_tasks=3, num_workers=6):
    ...
    #raw_quals = [[random.randint(5, 15) for _ in range(num_workers)] for _ in range(num_tasks)]
    raw_costs = [
        [26, 24, 19, 25, 17, 16, 15, 14, 13, 12],  # 任务1对应每个工人
        [10, 18, 12, 27, 16, 15, 14, 13, 12, 11],  # 任务2
        [16, 17, 11, 22, 19, 18, 17, 16, 15, 14],  # 任务3
        [26, 27, 21, 22, 29, 28, 27, 26, 25, 24],  # 任务4
        [18, 17, 16, 15, 14, 13, 12, 11, 10, 9],  # 任务5
    ]

    raw_quals = [
        [5, 8, 6, 14, 15, 16, 17, 18, 19, 20],   # 任务1
        [14, 8, 14, 8, 7, 6, 5, 4, 3, 2],  # 任务2
        [15, 10, 11, 9, 5, 4, 3, 2, 1, 10],   # 任务3
        [15, 14, 13, 12, 11, 10, 9, 8, 7, 6],  # 任务4
...
def run_moeo_wcd(pubkey, enc_costs, enc_quals,
    ...
    for gen in tqdm(range(num_iter), desc="🌱 进化轮"):
        ...
        while len(new_population) < pop_size:
            parent_idx = random.randrange(len(new_population))
