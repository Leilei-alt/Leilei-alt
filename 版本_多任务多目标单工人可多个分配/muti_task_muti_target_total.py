...
# 随机解生成与变异
# -------------------------------
def generate_random_matrix(num_tasks, num_workers):
    return [[random.randint(0, 1) for _ in range(num_workers)] for _ in range(num_tasks)]
...
def mutate_matrix(x, mutation_prob=0.1):
    """
    多任务分配矩阵的变异函数。
    :param x: List[List[int]]，T×N 分配矩阵
    :param mutation_prob: 每个位点变异概率（默认0.1）
    :return: 新的变异个体
    """
    T = len(x)
    N = len(x[0])
    ...
    for t in range(T):
        ...
        for i in range(N):
            if random.random() < mutation_prob:
                new_row.append(1 - x[t][i])  # 翻转位
            else:
                new_row.append(x[t][i])
...
def run_moeo_wcd(pubkey, enc_costs, enc_quals,
    ...
    """
    多任务版本的密文优化主函数（MOEO-WCD）。
    :param num_tasks: 任务数量 T
    :param min_assign: 每个任务最少分配工人数
    """

    N = len(enc_costs)  # 工人数量
    T = num_tasks       # 任务数量

    # 🧬 初始化种群（每个个体是 T×N 矩阵）
    population = [generate_random_matrix(T, N) for _ in range(pop_size)]
    ...
    for gen in range(num_iter):
        ...
        for x in population:
            ...
            valid = True
            for t in range(T):
                enc_task_sum = pubkey.encrypt(0)
                for i in range(N):
                    if x[t][i] == 1:
                        enc_task_sum += pubkey.encrypt(1)

                # 密文比较：判断是否 >= min_assign
                enc_mu = secure_compare(enc_task_sum, pubkey.encrypt(min_assign),
                                        pubkey, share0, share1, mu, n, n_sq)
    ...
    # 最后一代中选出“成本最小”的解（注意：此处目标为最小）
    best_idx = select_argmax_enc(
        [(i, obj_list[i][1]) for i in range(len(population))],  # 目标为 enc_cost
