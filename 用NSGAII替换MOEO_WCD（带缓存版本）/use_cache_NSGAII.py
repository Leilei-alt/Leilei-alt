...
def simulate_worker_upload(num_tasks=3, num_workers=6):
    ...
    # ✅ 加入工人成本权重（影响 SMUL 加权成本）
    raw_weights = [random.randint(1, 5) for _ in range(num_workers)]
    ...
    raw_max_dists = [random.randint(2000, 3000) for _ in range(num_workers)]
    ...
    with open("enc_worker_data.pkl", "wb") as f:
        pickle.dump({
            ...
            "raw_costs": raw_costs,
            "raw_quals": raw_quals,
            "raw_weights": raw_weights,     # ✅ 加入
            "enc_weights": enc_weights,     # ✅ 加入
...
def main():
    ...
    (pubkey, enc_costs, enc_quals, enc_weights,
     ...
     share0, share1, mu, n, n_sq) = prepare_data(num_tasks=num_tasks, num_workers=num_workers)
